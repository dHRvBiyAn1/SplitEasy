...
public class GroupController {
    ...
    /** Partial update from the group's settings page (rename / type / simplify-debts). */
    @PatchMapping("/{groupId}")
    public GroupResponse updateGroup(
            @CurrentUserId UUID userId,
            @PathVariable UUID groupId,
            @Valid @RequestBody UpdateGroupRequest request) {
        return groupService.updateGroup(userId, groupId, request);
    }

    /** Removes a member, or leaves the group when {@code memberId} is the caller. */
    ...
    /** Deletes the group and its expenses/payments. Creator only. */
    @DeleteMapping("/{groupId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteGroup(@CurrentUserId UUID userId, @PathVariable UUID groupId) {
